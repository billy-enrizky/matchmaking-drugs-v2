...
import { Filter, MapPin, ThumbsUp, MessageSquare, ChevronDown, ChevronRight, ExternalLink } from 'lucide-react';
...
const mockHospitals: Record<string, { name: string; address: string }> = {
  'hosp2': { name: 'General Hospital', address: '123 Main St, Anytown' },
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">My Drug Requests</h1>
          <p className="mt-2 text-gray-600">
            View and manage medication matches for your requests
          </p>
        </div>
        
        <div className="flex gap-3">
          <Link to="/request-drugs">
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-gray-500 mb-6">
            {distanceFilter > 0 || statusFilter 
              ...
              : "When we find matches for your medication requests, they'll appear here."}
          </p>
          <Link to="/request-drugs">
            <Button variant="primary">Request More Medications</Button>
          </Link>
        </Card>
      ) : (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex items-center text-sm text-gray-500">
                        ...
                        <span>
                          {mockHospitals[match.providerHospitalId].name} • {match.distance.toFixed(1)} km away
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {expandedMatch === match.id && (
                <div className="border-t border-gray-200 px-4 py-5 sm:px-6 bg-gray-50">
                  <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Provider Hospital</dt>
                      <dd className="mt-1 text-sm text-gray-900">{mockHospitals[match.providerHospitalId].name}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Address</dt>
                      <dd className="mt-1 text-sm text-gray-900">{mockHospitals[match.providerHospitalId].address}</dd>
                    </div>
                    <div>
